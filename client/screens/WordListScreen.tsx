...
export default function WordListScreen() {
  ...
  const filteredWords = useMemo(() => {
    switch (filter) {
      case "memorized":
        return groupWords.filter((w) => w.isMemorized && (w.unmemorizedCount || 0) === 0);
      case "unmemorized":
        return groupWords.filter((w) => (w.unmemorizedCount || 0) > 0);
      default:
        return groupWords;
    }
  }, [groupWords, filter]);
  ...
  const stats = useMemo(() => {
    const memorized = groupWords.filter((w) => w.isMemorized && (w.unmemorizedCount || 0) === 0).length;
    const unmemorized = groupWords.filter((w) => (w.unmemorizedCount || 0) > 0).length;
